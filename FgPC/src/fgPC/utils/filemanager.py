...
Date:           2024-05-14
Description:    The filemanager handles the creation and deletion of folders 
                for the results.
...
def create_result_folder(filepath, modelStr,tag = "", max_res_folders = None, max_daily_folders = None, archive = False):
    ...
    # Folder path
    if archive:
        path = filepath + modelStr + "Results/Archive/" + d1
    else:
        path = filepath + modelStr + "Results/" + d1
    ...
    if not archive:
        if max_daily_folders is not None:
            date_format = '%Y-%m-%d'
            remove_folders(filepath+ modelStr + "Results", max_daily_folders, date_format)
...
def save_script(filepath, filename, modelStr, tag = "", max_daily_folders = None, max_res_folders = None, archive = False):
    target_folder_path = create_result_folder(filepath, modelStr, tag, max_res_folders, max_daily_folders, archive)
